...
<mujoco model="table_tennis">
    ...
    <worldbody>
        <!-- Ground -->
        <geom name="floor" type="plane" size="10 10 0.1" rgba="0.4 0.4 0.4 1"/>
...
class KukaTennisEnv(gym.Env):
    ...
    def draw_line(self, start, end, line_k, color=(1.0, 0.0, 0.0, 1.0)):
        target_geom_id = mj.mj_name2id(self.model, mj.mjtObj.mjOBJ_BODY, 'line{}'.format(line_k))
        target_geom_id_ = mj.mj_name2id(self.model, mj.mjtObj.mjOBJ_GEOM, 'line{}_geom'.format(line_k))
        # self.line_k += 1
        # self.data.geom_xpos[target_geom_id] = np.array(pose)
    ...
    def draw_human_pose(self, pose, init_i=0,color=(0.0, 1.0, 0.0, 1.0)):
        for i in range(len(SKELETON)):
            start = pose[SKELETON[i][0]]
            end = pose[SKELETON[i][1]]
            self.draw_line(start, end, i+init_i, color)
    ...
    def calc_target_racket_pose(self,ball_pos,ball_vel,bounce_factor=1,table_z=0.66,g = -9.81,x_target=TABLE_SHIFT+1.37/2.,y_target=0.,x_gantry=-0.35):
        ...
        theta_z = np.pi/2.#np.arctan2(z,y)
        x_axis = np.array([0.,np.cos(theta_z),np.sin(theta_z)])
        x_axis[0] = (-x_axis[1]*z_axis[1] - x_axis[2]*z_axis[2])/z_axis[0]
        x_axis = x_axis/np.linalg.norm(x_axis)
        y_axis = np.cross(z_axis,x_axis)
    ...
    def render(self, mode="human"):
        ...
        if self.viewer is None:
            ...
            self.viewer.cam.distance = 5.21#4.1
        ...
        #     self.viewer.cam.azimuth = 90.68 + (162-90.68)*(self.current_step-200)/200
        #     self.viewer.cam.elevation = -24.42 + (24.42-18.4)*(self.current_step-200)/200
        #     self.viewer.cam.distance = 7.21 + (5.21-7.21)*(self.current_step-200)/200
        #     self.viewer.cam.lookat = np.array([1.43,0.2,0.41]) + (np.array([-0.04,0.124,0.116])-np.array([1.43,0.2,0.41]))*(self.current_step-200)/200
        # self.viewer.render()
