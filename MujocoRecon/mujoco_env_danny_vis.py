...
# from mujoco import mjv, mjr, mju
...
<mujoco model="table_tennis">
    <include file="iiwa14_gantry_vis.xml"/>
    <compiler angle="radian" />
    <option timestep="0.01" gravity="0 0 -9.81" />
    <worldbody>
        <!-- Ground -->
        <geom name="floor" type="plane" size="10 10 0.1" rgba="0.8 0.8 0.8 1"/>
        <geom name="gantry_range" type="plane" pos="-0.5 0 0.05" size="1 1 0.1" rgba="0.2 0.8 0.2 1" contype="0" conaffinity="0"/>
        ...
        <body name="C" pos="0 0 1.26" quat="0 0.7068252 0 0.7073883">
            <geom name="cylinder1" type="cylinder" pos="0.2 0 0" size="0.10 0.015" rgba="0 1 0 0.3" contype="0" conaffinity="0"/>
            <geom name="handle2" type="cylinder" pos="0.05 0 0" size="0.02 0.05" quat="0 0.7068252 0 0.7073883" rgba="0 0 1 0.3" contype="0" conaffinity="0"/>
        </body>
        <body name="G" pos="0 0 1.26" quat="0 0.7068252 0 0.7073883">
            <geom name="cylinder2" type="cylinder" pos="0.2 0 0" size="0.10 0.015" rgba="0 1 0 0.3" contype="0" conaffinity="0"/>
            <geom name="handle3" type="cylinder" pos="0.05 0 0" size="0.02 0.05" quat="0 0.7068252 0 0.7073883" rgba="0 0 1 0.3" contype="0" conaffinity="0"/>
        </body>
        <!-- Table -->
        ...
        <body name="net" pos="1.5 0 0.7" euler="0 0 0"> <!-- Position and rotate net -->
            <!-- Net surface -->
            <geom name="net_geom" type="box" size="0.01 0.7625 0.04" rgba="1 1 1 1" friction="0 0 0" solref="0.00001 1" solimp="0.99 0.9999 0.00001" />
        </body>
        <body name="sphere0" pos="2.18 0.04 0.66" euler="0 0 0"> <!-- Position and rotate net -->
            <geom name="sphere0_geom" type="sphere" size="0.04 0.04 0.04" rgba="0 0.6 0.3 0.7" contype="0" conaffinity="0" />
        </body>
        """
for i in range(n_lines):
    xml += """
        <body name="line{}" pos="1.5 {} 0.65" euler="0 0 0"> <!-- Position and rotate net -->
            <geom name="line{}_geom" type="cylinder" size="0.01 0.01 0.01" rgba="1 0 0 0.7" contype="0" conaffinity="0" />
        </body>
    """.format(i,i/n_lines*1.37-0.685,i)

for i in range(n_lines):
    xml += """
        <body name="vis_{}" pos="0 0 1.26" quat="0 0.7068252 0 0.7073883">
            <geom name="cylinder_{}" type="cylinder" pos="0.2 0 0" size="0.10 0.015" rgba="0 1 0 0.3" contype="0" conaffinity="0"/>
            <geom name="handle_{}" type="cylinder" pos="0.05 0 0" size="0.02 0.05" quat="0 0.7068252 0 0.7073883" rgba="0 0 1 0.3" contype="0" conaffinity="0"/>
        </body>
    """.format(i,i,i)
...
class KukaTennisEnv(gym.Env):
    ...
    def set_C_pose(self,pose):
        # print("Req:", pose)
        target_geom_id = mj.mj_name2id(self.model, mj.mjtObj.mjOBJ_BODY, 'C')
        print("Setting C pose", pose,target_geom_id)
    ...
    def set_G_pose(self,pose):
        # print("Req:", pose)
        pose = np.array(pose).copy()
        target_geom_id = mj.mj_name2id(self.model, mj.mjtObj.mjOBJ_BODY, 'G')

        # self.data.geom_xpos[target_geom_id] = np.array(pose)
        print("Setting G pose", pose,target_geom_id)
        self.model.body_pos[target_geom_id] = pose[:3]
        self.model.body_quat[target_geom_id] = pose[[6,3,4,5]]

    def set_target_pose(self,pose):
        # print("setting target pose", pose)
        # print("Req:", pose)
    ...
    def update_vis_pose(self,pose):
        ...
        print("Setting vis pose", pose,target_geom_id)
        # self.data.geom_xpos[target_geom_id] = np.array(pose)
        self.model.body_pos[target_geom_id] = pose[:3]
        self.model.body_quat[target_geom_id] = pose[[6,3,4,5]]
    
    def update_vis_pose_(self,pose):
        # Update the cylinder geom position
        target_geom_id = mj.mj_name2id(self.model, mj.mjtObj.mjOBJ_BODY, 'vis_{}'.format(self.curr_pose_i))
        print("Setting vis pose", pose,target_geom_id)
        # self.data.geom_xpos[target_geom_id] = np.array(pose)
        self.model.body_pos[target_geom_id] = pose[:3]
        self.model.body_quat[target_geom_id] = pose[[6,3,4,5]]
    ...
    def step(self, action):
        ...
        self.data.ctrl[:] = np.array(action[2:])/2. + np.array(self.data.qpos[2:9])
    ...
    def draw_line(self, start, end, color=(1.0, 0.0, 0.0, 1.0)):
        target_geom_id = mj.mj_name2id(self.model, mj.mjtObj.mjOBJ_BODY, 'line{}'.format(self.line_k))
        target_geom_id_ = mj.mj_name2id(self.model, mj.mjtObj.mjOBJ_GEOM, 'line{}_geom'.format(self.line_k))
        self.line_k += 1
        # self.data.geom_xpos[target_geom_id] = np.array(pose)
        self.model.body_pos[target_geom_id] = (start+end)/2
        x_axis = end-start
        x_axis = x_axis/np.linalg.norm(x_axis)
        z_axis = np.array([0.,0.,1.])
        z_axis[0] = (-x_axis[1]*z_axis[1] - x_axis[2]*z_axis[2])/x_axis[0]
        z_axis = z_axis/np.linalg.norm(z_axis)
        y_axis = np.cross(z_axis,x_axis)
        Rot = np.array([y_axis,z_axis,x_axis]).T
        # print(Rot)
        r = R.from_matrix(Rot)
        q = r.as_quat()
        self.model.body_quat[target_geom_id] = q[[3,0,1,2]]
        self.model.geom_size[target_geom_id_] = np.array([0.015,np.linalg.norm(end-start)/2.,0.])
        self.model.geom_rgba[target_geom_id_] = color

    def draw_trajectory(self,waypoints, color=(1.0, 0.0, 0.0, 1.0)) :
        print("Draw traj")
        for i in range(len(waypoints) - 1) :
            # print("Here",waypoints[i],waypoints[i+1])
            self.draw_line(waypoints[i], waypoints[i + 1])
        print("Done")
    ...
    def reset_target_(self):
        self.curr_target = np.array([0.,0.,0.,0.,0.,0.,0.])
        self.curr_target[0] = np.random.uniform(-1.,0.)
        self.curr_target[1] = np.random.uniform(-1.,1.)
        self.curr_target[2] = np.random.uniform(0.75,1.05)
        xr,yr,zr = np.random.uniform(-1,1,3)*0.5
        dx = 2.1 - self.curr_target[0]
        dy = 0. - self.curr_target[1]
        dz = 0.66 - self.curr_target[2]
        z_axis = np.array([1.,dy/dx+yr,dz/dx+zr])
        z_axis = z_axis/np.linalg.norm(z_axis)
        theta_z = np.arctan2(self.curr_target[2],self.curr_target[1])+xr*0.5
        x_axis = np.array([0.,np.cos(theta_z),np.sin(theta_z)])
        x_axis[0] = (-x_axis[1]*z_axis[1] - x_axis[2]*z_axis[2])/z_axis[0]
        ...
        q = r.as_quat()
        self.curr_target[3:7] = q
        self.update_vis_pose_(self.curr_target)

    def calc_target_racket_pose(self,ball_pos,ball_vel,bounce_factor=1,table_z=0.67,g = -9.81,x_target=TABLE_SHIFT+1.37/2.,y_target=0.,x_gantry=-0.4):
        ...
        yzr = np.meshgrid(np.linspace(-1.5,1.5,100),np.linspace(-1.5,1.5,100))
        ...
        x_hits = pos[0] - 0.1 + ball_reflected_vels[0]*t_hits
        y_hits = pos[1] + ball_reflected_vels[1]*t_hits
        costs = (x_hits-x_target)**2 + (y_hits-y_target)**2
        idx = np.argmin(costs)
        z_axis = xyzr[idx]
        traj = []
        ball_reflected_vels = ball_reflected_vels[:,idx]
        for t in np.arange(0.,t_hits[idx],0.01):
            traj.append([pos[0] - 0.1 + ball_reflected_vels[0]*t,pos[1] + ball_reflected_vels[1]*t,pos[2]+0.2 + ball_reflected_vels[2]*t + 0.5*g*t*t])
        traj = np.array(traj)
        self.draw_trajectory(traj,color=(0.0, 1.0, 1.0, 0.4))
        ...
        pos[2] += 0.1
        pos[1] += 0.1
        self.set_G_pose(np.concatenate([pos,q]))
    ...
    def render(self, mode="human"):
        ...
        if self.viewer is None:
            ...
            self.viewer.cam.azimuth = 162
            self.viewer.cam.elevation = -18.4
            self.viewer.cam.distance = 5.
            self.viewer.cam.lookat = np.array([-0.04,-0.124,0.116])
