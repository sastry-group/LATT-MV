...
### Using the trained model(s) for inference on some examples

You can run the inference script on some examples included in the `data` directory. The script will generate a video with the predicted trajectory and the ground truth trajectory in rec.gif. You can run the script with

```bash
python3 inference.py 
```

This will also save the predicted trajectories from individual transformers in release_data/match939/match939_65/match939_65_pred.npy. You can change the video name in the script to run inference on other videos. These predicted trajectories can be better visualized with scripts in visualize/ directory explained later.
...
You can visualize the reconstructed gameplay with the following command. 

```bash
python3 visualize_recon.py --match <match_id> --rally <rally_id>
```

You can visualize the predicted trajectory from phase 2 with the following command. 

```bash
python3 visualize_pred.py --match <match_id> --rally <rally_id> 
```
