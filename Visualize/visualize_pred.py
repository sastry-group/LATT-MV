...
def render_projections_on_frames(v, frame_projections, streak_length=5):
    ...
    for t, frame in enumerate(frames):
        ...
        for i in range(6):
            if f"ball_pred_{i}" in frame_projections[t]:
                # print(i)
                b_pred_trail[str(i)].append(frame_projections[t][f"ball_pred_{i}"])
                if len(b_pred_trail[str(i)]) > streak_length:
                    b_pred_trail[str(i)].pop(0)
        ...
        for i in range(6) :
            if len(b_pred_trail[str(i)]) > 1:
                points = np.array(b_pred_trail[str(i)], dtype=np.int32)
                if i in group_list:
                    cv2.polylines(frame_with_projections, [points], False, (0, 255, 255), thickness=1 * RESIZE)
        ...
        for i in range(6) :
            if f"ball_pred_{i}" in frame_projections[t]:
                # print(i)
                b_pred_current = frame_projections[t][f"ball_pred_{i}"]
                if i in group_list:
                    cv2.circle(frame_with_projections, tuple(map(int, b_pred_current)), 2 * RESIZE, (0, 255, 255), -1)
        ...
        for i in range(6):
            if f"conf_box{i}" in frame_projections[t]:
                points = np.array(frame_projections[t][f"conf_box{i}"], dtype=np.int32)
                overlay = frame_with_projections.copy()
...
def main():
    ...
    parser.add_argument('--rally', type=int, required=True,
    ...
    v = reconstruct("/home/pingpong-daniel/data_danny", f"match{a}", f"match{a}_{b}", verbose=False, render=False)
    preds = np.load(f'/home/pingpong-daniel/data_danny/match{a}/match{a}_{b}/match{a}_{b}_pred.npy')
    # print(preds.shape)
    preds[:,:,0] = preds[:,:,0] * -1
    preds[:,:,1] = preds[:,:,1] * -1
    p1_hits = v.player1_hits
    p2_hits = v.player2_hits
    _t = p1_hits[0] + len(preds[0]) - 5
    k = 0
    while k<len(p2_hits) and p2_hits[k] < _t:
        k += 1
    t_ = p2_hits[k-1] - 7
    # print("Here 0")
    v.frames = v.frames[:p1_hits[0]+len(preds[0])]
    ...
    xs = preds[:,-1,0].copy()
    # Get indices of min 4 xs
    group_list = []
    for i in range(6):
        group_list.append(np.argmin(xs))
        xs[np.argmin(xs)] = 1000
    
    for t in T:
        if t>=t_:
            conformal_box_lines = []
            var = np.var(preds[group_list,t-p1_hits[0]], axis=0)
            mean = np.mean(preds[group_list,t-p1_hits[0]], axis=0)
            face_pts = [[1,1],[1,-1],[-1,-1],[-1,1]]
            for i in range(2):
                k = 2*i-1
                for j in range(3):
                    conformal_box_faces = []
                    for pt in face_pts:
                        if j ==0:
                            p = mean + VAR_FACTOR * np.sqrt(var) * np.array([k, pt[0], pt[1]])
                            conformal_box_faces.append(p)
                        elif j == 1:
                            p = mean + VAR_FACTOR * np.sqrt(var) * np.array([pt[0], k, pt[1]])
                            conformal_box_faces.append(p)
                        else:
                            p = mean + VAR_FACTOR * np.sqrt(var) * np.array([pt[0], pt[1], k])
                            conformal_box_faces.append(p)
                    conformal_box_lines.append(np.array(conformal_box_faces))
            for i in range(6):
                frame_projections[t]["conf_box"+str(i)] = [tuple(map(int, project_point(p, v, t))) for p in conformal_box_lines[i]]
            for i in range(0, len(preds)):
                # print(preds[i][t-p1_hits[0]],v.ball[t])
                frame_projections[t]["ball_pred_"+str(i)] = project_point(preds[i][t-p1_hits[0]], v, t)
